%% substitute_NonCentroSymm_atom.m
...
% * atom is the atom struct. This is the non-centrosymmetric version.
%
%% Version
% 3.00
%
...
%
%% Examples
% # atom = substitute_NonCentroSymm_atom(atom,Box_dim,5,'Al','Mgo',5.5) % Basic input arguments
% # atom = substitute_NonCentroSymm_atom(atom,Box_dim,5,'Al','Mgo',5.5,2,'Si','Al',5.5) % Will perform both octahedral and tetrahedral replacements
% # atom = substitute_NonCentroSymm_atom(atom,Box_dim,5,'Al','Mgo',5.5,2,'Si','Al',5.5,-2.5,12.5,3) % Only subst. between z>-2.5 and z<12.5 in the z-direction (3).
%
function atom = substitute_NonCentroSymm_atom(atom,Box_dim,NumOctSubst,O1,O2,minO2O2_dist,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if NumOctSubst>0
    ...
    i=1; nSubSite=0; Oct_subst_index=[];%rand_O1_Index(1);
    while nSubSite<=NumOctSubst
        ind_O2=find(strcmp([O1_atom.type],O2));
        O=intersect(ind_O2,find(O1_dist_matrix(rand_O1_Index(i),:)<minO2O2_dist));
        if length(O)<1 && nSubSite<NumOctSubst && lolimit<O1_data(rand_O1_Index(i),dimension) && hilimit>O1_data(rand_O1_Index(i),dimension)
            %             O1_data(rand_O1_Index(i),2);
            if nSubSite < NumOctSubst
...
if NumOctSubst>0
    if nSubSite == NumOctSubst
        ...
        try
            nSubSite
            assignin('caller','nSubSite',nSubSite)
        catch
            disp('No first subst...')
        end
