%% direction_atom.m
% * This function tries to find all angles between the  atom1 and atom2 
% versus a given vector direction_vec, a nx3 unit vector (where n typically 
% is 1). One optional argument like 'reverse' reverses the angle_limit from 
% max to min angle
%
%% Version
% 3.00
...
%
%% Examples
% # atom1 = direction_atom(atom1,atom2,Box_dim,direction_vec,1.25,2.25,150)
% # atom1 = direction_atom(atom1,atom2,Box_dim,direction_vec,1.25,2.25,150,1)
% # atom1 = direction_atom(atom1,atom2,Box_dim,direction_vec,1.25,2.25,150,1,'reverse')

function atom1 = direction_atom(atom1,atom2,Box_dim,direction_vec,rmaxshort,rmaxlong,angle_limit,varargin)
%%
...
for i=1:size(atom1,2)
    ...
    for v=1:size(neigh_ind,1)
        for w=1:size(direction_vec,1) % From v or from 1?
            angle=rad2deg(atan2(norm(cross(neigh_vec(v,:),direction_vec(w,:))),dot(neigh_vec(v,:),direction_vec(w,:))));
            if nargin > 7
            ...
            if angle > 0 && angle_lo < angle_hi
                if neigh_ind(v,1) < direction_vec(w,1)
                    Angle_index(a,1)= atom2(neigh_ind(v,1)).index;
                    Angle_index(a,2)= solute_index;
                    Angle_index(a,3)= 0; % atom2(direction_vec(w,1)).index;
                    Angle_index(a,4)= angle;
                    Angle_index(a,5:7)= neigh_vec(v,:);
                    Angle_index(a,8:10)= direction_vec(w,:);
                    Angle_index(a,11)= neigh_dist(v);
                    Angle_index(a,12)= direction_vec(w);
                    a=a+1;
                else
                    Angle_index(a,1)= 0; % atom2(direction_vec(w,1)).index;
                    Angle_index(a,2)= solute_index;
                    Angle_index(a,3)= atom2(neigh_ind(v,1)).index;
                    Angle_index(a,4)= angle;
                    Angle_index(a,5:7)= direction_vec(w,:);
                    Angle_index(a,8:10)= neigh_vec(v,:);
                    Angle_index(a,11)= direction_vec(w);
