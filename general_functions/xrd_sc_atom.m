%% xrd_sc_atom.m
% * This experimental function calculates theoretical XRD patterns from a 
% .pdb|.gro etc. and differs from xrd_atom in that it tries to implement 
% the supercell concept by Ufer, 2008. Does it work?
...
function [twotheta,intensity] = xrd_sc_atom(varargin)
...
for i=num_hkl
    ...
    if size(Box_dim,2) == 9
        lx=Box_dim(1);    ly=Box_dim(2);    lz=Box_dim(3)*i;
        xy=Box_dim(6);    xz=Box_dim(8)*i;  yz=Box_dim(9)*i;
    elseif size(Box_dim,2) == 3
        lx=Box_dim(1);    ly=Box_dim(2);    lz=Box_dim(3)*i;
        xy=0;    xz=0;    yz=0;
    end
    
    x=[frac.xfrac]'./i;y=[frac.yfrac]'./i;z=[frac.zfrac]'./i;
    ...
    l=l_values_temp;
    assignin('caller','l',l);
    ind_hk0=find(h==0&k==0);
    ind_l0=setdiff(1:numel(l),ind_hk0);
%     l(ind_l0)=l(ind_l0)*0;
    l(ind_hk0)=l(ind_hk0)*num_hkl;
%     assignin('caller','lsc',l);
%     h=zeros(1,numel(h));
%     k=zeros(1,numel(k));
%     assignin('caller','supercell_ind',supercell_ind);
%     assignin('caller','h',h);
%     assignin('caller','k',k);
   
    hkl=[h' k' l'];
    
    %     assignin('caller','hkl1',hkl);
...
%         structure_factor=structure_factor+scattering_factor.*occupancy(n).*exp(2*pi*1i.*(h*x(n)+k*y(n)+l*z(n)));
    ...
    %         size('tot_intensity')
    %         size('intensity')
    %         tot_intensity=tot_intensity+intensity*exp(-i/4);
    %     end
    
end

% intensity=tot_intensity;
%
% plot(exp_twotheta,intensity,'k');
