...
% mclf code. Not really tested alot..
%
%% Version
% 3.00
%
...
%
%% Examples
% # atom = import_mclf_dispersion('XYZ_even_tempered_net_atomic_charges.xyz')
%
function atom = import_mclf_dispersion(varargin)
...
for i=1:nAtoms
    ...
    Y(i) = XYZ_string(3);
    Z(i) = XYZ_string(4);
    mclf_C6(i) = XYZ_string(5);

end
fclose(filetempID);
mclf_C6=str2double(mclf_C6);
...
for i=1:nAtoms
    ...
    atom(i).vy=0;
    atom(i).vz=0;
    atom(i).C6_aubhr6=mclf_C6(i);
    atom(i).C6_kJmolnm6=mclf_C6(i)*4.35974472220718E-18*6.022E+23/1000*(0.052917721^6);
end
...
for i=1:size(Atom_labels,2)
    ...
    Ave_C6_aubhr6=mean([atom(ind).C6_aubhr6]);
    Ave_C6_kJmolnm6=mean([atom(ind).C6_kJmolnm6]);
    Ave_C6_kJmolnm6=mean([atom(ind).C6_kJmolnm6]);
    All_C6=[All_C6; [Atom_labels(i) {Ave_C6_aubhr6} {Ave_C6_kJmolnm6}]];
end

writecell(All_C6,'All_Ave_C6.dat');
