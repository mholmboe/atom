%% import_atom_pqr.m
% * This function import .pqr files into the atom struct
...
%
%% Examples
% # atom = import_atom_pqr('molecule.pqr')
% # atom = import_atom_pqr('molecule.pqr',[10 5 2])
% # atom = import_atom_pqr('molecule.pqr',[10 5 0],[35.24 24.23 52.23])
%
function atom = import_atom_pqr(filename,varargin)
...
% See http://deposit.rcsb.org/adit/docs/pqr_atom_format.html
...
for i = 1:length(data)
    ...
    if ((length(line)>=4) && (strcmp(line(1:4), 'ATOM') || strcmp(line(1:6), 'HETATM')))
        ...
        atom(j).vy = NaN;
        atom(j).vz = NaN;
        atom(j).charge = str2double(line(56:63));
        atom(j).radius = str2double(line(64:69));
        atom(j).element = {strtrim(line(80:83))};
        atom(j).formalcharge = {strtrim(line(84:86))};
...
assignin('caller','charge',[atom.charge])
assignin('caller','radius',[atom.radius])
