...
### Create em.mdp file ########################################
cat << EOF >| "$sim".mdp
define		= -DFLEXIBLE -D$watermodel -DPOSRES_INTERFACE -DPOSRES_Ion -DPOSRES_noH 
; Run parameters
integrator		= sd		; stochastic integrator
...
$run_grompp -f ./"$sim".mdp -c pre"$sim".gro -r pre"$sim".gro -p $gmx_run/$moldir/topol.top -n $gmx_run/$moldir/index.ndx -o "$sim".tpr -pp -po #-maxwarn 1
